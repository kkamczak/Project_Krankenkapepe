...
class Game:
    def __init__(self):
        # Overworld creation
        self.main_menu = Main_Menu(SCREEN, ['Start', 'Exit'], self.create_level, BUTTON_FONT, self.exit_game, 0)
    ...
    def create_pause(self):
        self.pause = Pause(SCREEN, ['Return', 'Main Menu', 'Exit'], BUTTON_FONT, self.exit_game, self.stop_pause, self.create_main_menu, 0)
        self.status = 'pause'
    ...
    def create_death_scene(self):
        self.death_scene = Death_Scene(SCREEN, ['Respawn', 'Main Menu', 'Exit'], NORMAL_FONT, DEATH_FONT, self.create_main_menu, self.create_level, self.exit_game, 70)
        self.status = 'dead'
...
while True:
    ...
    if game.status == 'main_menu':
        ...
        SCREEN.fill(SKY)
        game.paused = False
    elif game.status == 'pause' and game.paused == False:
        print_mask()
        game.paused = True
